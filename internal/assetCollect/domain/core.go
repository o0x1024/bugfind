...
func ColletDomainByInterface(task *response.Task) {
	for _, vd := range task.Domains {
		go func(dom response.Domain) {
			updata := &request.InputDataStruct{}
			updata.CompanyId = dom.CId
			updata.DataType = "domain"
			for _, v := range dom.RootDomains {
				...
				runnerInstance, err := runner.NewRunner(&runner.Options{
					...
					ProviderConfig:     "provider.yaml",
					Silent:             true,
				})

				buf := bytes.Buffer{}
				err = runnerInstance.EnumerateSingleDomain(v, []io.Writer{&buf})
				if err != nil {
					log.Fatal(err)
				}
