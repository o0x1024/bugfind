...
func ParseOptions() *Options {
	...
	if *resolvers != "" {
		...
		if err != nil {
			log.Println("读取resolvers文件失败:%s\n", err.Error())
			return nil
		}
		if len(rs) == 0 {
			log.Println("resolvers文件内容为空\n")
			return nil
		}
	...
	case "k":
		rate *= 1000
	default:
		log.Println("unknown bandwith suffix '%s' (supported suffixes are G,M and K)\n", suffix)
		return nil
	}
